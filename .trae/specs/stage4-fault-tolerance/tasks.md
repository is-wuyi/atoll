...
- [x] Task 9: CI + 构建 + 5 机部署
  ...
  - [x] 9.2 5 台机器滚动更新二进制（md5 一致）并重启服务；验证节点注册/心跳正常、master 扫描器日志出现（27348 关机排除，其余 4 台部署完成）
...
- [x] Task 10: 实机验收 scripts/acceptance-fault.sh
  ...
  - [x] 10.2 真实验收 ALL PASS（2026-08-21，12/12；从 mac 执行，107 被群晖自动封锁 SSH）。含防假 PASS 前置（节点停止后 healthz 复核）、GC 闭环（PUT 假孤儿→dry-run 报告→execute→验证消失）、内容级比对。master 日志交叉印证判死/回池/GC。27348 开机后可补 3 节点"换机重建"场景
